...
def sample_image(n_row, batches_done):
    ...
    save_image(gen_imgs.numpy(), "logs/images/%d.png" % batches_done, nrow=n_row)
...
for epoch in range(opt.n_epochs):
    for i, (imgs, labels) in enumerate(dataloader):
        ...
        if i  % 50 == 0:
            ...
            writer.add_scalar('losses/d_real_loss', d_real_loss.item(), epoch * len(dataloader) + i)
            writer.add_scalar('losses/d_fake_loss', d_fake_loss.item(), epoch * len(dataloader) + i)
            writer.add_scalar('losses/d_loss', d_loss.item(), epoch * len(dataloader) + i)
            writer.add_scalar('losses/g_loss', g_loss.item(), epoch * len(dataloader) + i)
...
number = '18792492077' #TODO: 写入你注册时绑定的手机号（字符串类型）,此处隐私
n_row = len(number)
